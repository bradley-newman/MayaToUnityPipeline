...
The following is my example of a basic art pipeline from Maya to Unity. The focus is fast UI development, and automating repetitive tasks. It features:
* Maya Dockable Main Window UI code for:
    * Loading a UI from code generated in Qt Designer.
    * Creating, Showing, and Destroying a Dockable Window safely.
...
* Automating exporting assets to the correct folders in the Unity project.
* Baking animation and exporting only the skeleton without the rig.
* Assigning attributes in Maya that can be parsed in Unity to automate animation import settings for looping animations, and automate prefab creation settings for static meshes.
...
To view my example Main Window open the `main_view.ui` file. 
...
The function returns a `DockableMainWindow` object. The `ui_main_window` parameter should be a reference to the `Ui_MainWindow` class generated by Qt Designer.
...
if mode is mp.UI_Creation_Mode.DEFAULT:
    if workspace_control_exists(workspace_control_name):
        ...
        for child_widget in mayaMainWindow.findChildren(QWidget):
    ...
    mp.debug_log(f"Restoring from Maya prefs...")
    ...
    mixinPtr = omui.MQtUtil.findControl(dockable_main_win.objectName())
...
if dockable_main_win == None:
    mp.debug_error("dockable_main_win == None", print_to_script_editor=True)

mp.debug_log(f"Finished running dockable_main_win_script()")
...
The Maya Pipeline Workspace Control is created and controlled with the following MVC pattern.


### Main App

The main entry point for the `MayaPipeline` object by calling the `open` function from Maya like so:
...
mp.open("Maya Pipeline", mp.UI_Creation_Mode.DEFAULT)
```

`mp.open` creates a `MayaPipeline` object that initializes the MVC.

If you are actively changing code, after running `memory_management.cleanup_memory()`, you can recreate the control like so:
...
mp.open("Maya Pipeline", mp.UI_Creation_Mode.RECREATE)
```

**[main_app.py](PyCharmProject/art_pipeline/maya/maya_pipeline/main_app/main_app.py)**

The `mp.open` function creates a MayaPipeline object:
...
This Model is the data container for various asset properties.
...
This View creates the Workspace Control, sets up the UI widgets, and connects them to the Controller functionality.
...
To solve for this the export location can be set in **Edit > Settings**, which is then saved in `Settings.json.` The **File > Export** command exports the asset to a directory structure that directly mirrors the Maya project. For cases where a user needs to manually select an export path, the **File > Export to Custom Location** can be used.
