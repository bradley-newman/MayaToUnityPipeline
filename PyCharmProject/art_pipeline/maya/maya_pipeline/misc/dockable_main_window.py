...
class DockableMainWindow(MayaQWidgetDockableMixin, QWidget):
    def __init__(self, title: str, ui_main_window:QMainWindow, parent=None):
        ...
        '''
        mp.debug_log(f"DockableMainWindow > __init__...")
...
def create_workspace_control_with_dockable_main_win(title: str,
    ...
    RESTORE_FROM_MAYA_PREFS mode is not meant to be called directly, and is only meant to be called from the uiScript command
    which saves the command in Maya prefs so when Maya restarts it can restore the control.
    '''
...
def get_dockable_main_win_child(parent:QWidget) -> DockableMainWindow:
    ...
    if parent is None:
        mp.debug_warning(f"parent is None, can't get dockable_main_win child", print_to_script_editor=True)
    else:
...
def create_workspace_control(title: str, dockable_main_win:DockableMainWindow):
    mp.debug_log(f"create_workspace_control...")
    ...
    ui_script = f"import maya_pipeline as mp\nmp.main_app.main_app.open('{title}', mp.UI_Creation_Mode.RESTORE_FROM_MAYA_PREFS)"
    mp.debug_log(f"showing workspace control...")
...
def workspace_control_exists(name:str) -> bool:
    exists = pm.workspaceControl(name, query=True, exists=True)
    if exists:
...
def delete_workspace_control_widgets(workspace_control_name: str):
    if workspace_control_exists(workspace_control_name):
        ...
        if workspace_control_widget is not None:
            ...
            mp.debug_warning(f"workspace_control_widget is None. Can't proceed with deleting child widgets.",
...
def delete_workspace_control(workspace_control_name:str):
    if workspace_control_exists(workspace_control_name):
        ...
        mp.debug_warning(f"Can't delete workspace control {workspace_control_name} because it doesn't exist.", print_to_script_editor=True)
