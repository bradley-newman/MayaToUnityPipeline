...
def get_current_scene_path() -> pathlib.Path:
    ...
    if current_scene_path.exists():
        ...
        mp.debug_warning(f"Current scene is not saved.", print_to_script_editor=True)
        return None
...
def get_nodes_in_namespace(namespace:str) -> list[pm.PyNode]:
    if pm.namespace(exists=namespace) is False:
        mp.debug_warning(f"Namespace {namespace} doesn't exist. Can't find nodes in this namespace.", print_to_script_editor=True)
        return None
    else:
        top_level_nodes = pm.ls(assemblies=True)
        nodes_in_namespace = []
