...
def _export_skinned_mesh(node:pm.PyNode, export_filepath:Path):
    mp.debug_log(f"Exporting Skinned Mesh...")
...
def _export_animation(node:pm.PyNode, export_filepath:Path):
    try:
        mp.debug_log(f"Exporting Animation...")

        # Find the rig
        rig_node = _get_descendent_of_asset_type(node, mp.AssetType.RIG)
        if not rig_node:
            mp.debug_error(f"Didn't find rig node.", print_to_script_editor=True)
            return
        ...
        if not rig_ref:
            mp.debug_error(f"No rig reference found.", print_to_script_editor=True)
            return
        ...
        if not skeleton_node:
            mp.debug_error(f"No skeleton found.", print_to_script_editor=True)
            return
        ...
        mp.debug_log(f"Finished animation export.")
